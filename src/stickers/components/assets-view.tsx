    ...
    <div className="flex h-full flex-col py-2">
      <div className="px-2">
        <Input
          size="sm"
          variant="default"
          placeholder={t.editor.stickers.searchPlaceholder}
          value={searchQuery}
          onChange={(e) => {
        ...
        <TabsList aria-label="Sticker categories">
          {Object.entries(STICKER_CATEGORIES).map(([key]) => (
            <TabsTrigger key={key} value={key}>
              {t.editor.stickers.categories[
                key as keyof typeof t.editor.stickers.categories
              ] || key}
            </TabsTrigger>
          ))}
        </TabsList>
  ...
  return (
    <div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
      ...
      <div className="flex flex-col gap-2 text-center">
        <p className="text-lg font-medium">{t.editor.stickers.emptySearch}</p>
...
function StickersContentView() {
  ...
  if (viewMode === "search") {
        ...
        <div className="flex flex-col gap-3 pb-4">
          ...
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground text-sm">
              {t.editor.stickers.resultsCount.replace(
                "{count}",
                searchResults.total.toString(),
              )}
            </span>
          </div>
    ...
    if (selectedCategory !== "all" && searchQuery) {
      return (
        <EmptyView
          message={t.editor.stickers.emptyResults.replace(
            "{query}",
            searchQuery,
          )}
        />
  ...
  if (!browseContent?.sections.length) {
    const categoryLabel =
      t.editor.stickers.categories[
        selectedCategory as keyof typeof t.editor.stickers.categories
      ] || selectedCategory;
    return (
      <EmptyView
        message={
          viewMode === "search"
            ? t.editor.stickers.emptyResults.replace("{query}", searchQuery)
            : selectedCategory === "all"
              ? t.editor.stickers.emptyAvailable
              : t.editor.stickers.emptyCategory.replace(
                  "{category}",
                  categoryLabel,
                )
        }
...
function StickerItem({
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.error(t.editor.stickers.addError);
    } finally {
      setIsAdding(false);
    }
  };

        ...
        <div className="px-3 pt-4 pb-0">
          <TabsList>
            <TabsTrigger value="sound-effects">
              {t.editor.sounds.tabs.effects}
            </TabsTrigger>
            <TabsTrigger value="saved">
              {t.editor.sounds.tabs.saved}
            </TabsTrigger>
          </TabsList>
        </div>
  ...
  return (
    <div className="mt-1 flex h-full flex-col gap-5">
      <div className="flex items-center gap-3">
        <Input
          placeholder={t.editor.sounds.searchPlaceholder}
          className="w-full"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            >
              {t.editor.sounds.commercialFilter}
            </DropdownMenuCheckboxItem>
            <div className="text-muted-foreground px-2 py-1.5 text-xs">
              {showCommercialOnly
                ? t.editor.sounds.commercialFilterActive
                : t.editor.sounds.commercialFilterInactive}
            </div>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            {!isLoading && !isSearching && displayedSounds.length === 0 && (
              <div className="text-muted-foreground text-sm">
                {searchQuery
                  ? t.editor.sounds.emptySearch
                  : t.editor.sounds.emptyAvailable}
              </div>
            )}
            {isLoadingMore && (
      ...
      <div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
        ...
        <div className="flex flex-col gap-2 text-center">
          ...
          <p className="text-muted-foreground text-sm text-balance">
            {t.editor.sounds.noSavedDescription}
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground text-sm">
          {t.editor.sounds.savedCount
            .replace("{count}", savedSounds.length.toString())
            .replace(
              "{unit}",
              savedSounds.length === 1
                ? t.editor.sounds.savedUnitSingle
                : t.editor.sounds.savedUnitMultiple,
            )}
        </p>
        <Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t.editor.sounds.clearAllTitle}</DialogTitle>
              <DialogDescription>
                {t.editor.sounds.clearAllDescription.replace(
                  "{count}",
                  savedSounds.length.toString(),
                )}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button variant="text" onClick={() => setShowClearDialog(false)}>
                {t.editor.sounds.cancel}
              </Button>
              <Button
                variant="destructive"
                onClick={async ({
    ...
    <div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
      ...
      <div className="flex items-center gap-3 pr-2">
        ...
        <Button
          ...
          title={
            isSaved ? t.editor.sounds.removeSaved : t.editor.sounds.saveSound
          }
        >
          <HugeiconsIcon
            icon={FavouriteIcon}
